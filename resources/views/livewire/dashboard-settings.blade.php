      ...
      <main class="mt-5 flex flex-col gap-4 overflow-y-auto">
        <section x-data="{save: false}" class="flex flex-col gap-5 border border-gray-200 rounded-lg p-4">
          <header class="flex justify-between items-center">
            <h2 class="font-medium text-gray-500">Payment Methods</h2>
              <!-- edit icon goes here -->
            <button x-show="! save" @click="$dispatch('edit-methods'); save = true" class="flex items-center justify-center gap-2 border border-gray-200 py-2 px-4 rounded-full text-gray-400 text-sm font-medium w-fit h-fit self-center hover:bg-gray-400 hover:text-white transition delay-150">
              Edit
            ...
            <!-- Save button -->
            <button x-show="save"  @click="$wire.updateUser(); $dispatch('save-methods'); save = false;" class="flex items-center justify-center gap-2 border border-gray-200 py-2 px-4 rounded-full text-gray-400 text-sm font-medium w-fit h-fit self-center hover:bg-gray-400 hover:text-white delay-150">
              Save
          ...
          <section class="text-gray-500">
            <div>
              ...
              <input 
              wire:model="momo"
              @save-methods.window="$el.setAttribute('disabled', 'true'); $el.setAttribute('class', 'bg-white p-0 my-0 font-medium bg-white text-gray-600 border rounded-md p-2')" 
              @edit-methods.window="$el.removeAttribute('disabled'); $el.setAttribute('class', 'p-2 border border-blue-400 rounded-md outline-blue-400 placeholder:text-blue-400')" id="momo"
              class="font-medium bg-white text-gray-600 border rounded-md p-2" 
              type="text" 
              name="momo" 
              placeholder="+233" 
              disabled 
